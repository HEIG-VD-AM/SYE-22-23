...
int virt_to_pte(uint16_t vaddr, uint16_t *pte)
{
    ...
    // Vérification que le pointeur ne soit pas null
    if(!pte) {
        printf("PTE pointer is null");
        return -1;
    }

    *pte = page_table[(vaddr >> SHIFT_RIGHT) & MASK];

    return 0;


#if MEMORY_SIZE_SMALL // A completer étape 4
    // Pas utilisé, tout est pré-processé
#else
...
int get_byte(uint32_t vaddr, uint8_t *byte) {
    ...
    /*A COMPLETER*/
    uint16_t pte;
    if(virt_to_pte(vaddr, &pte) < 0) {
        return -1;
    }

#if MEMORY_PTE_ATTRIBUTES_ACTIVE //A completer étape 3

    // Vérifie si la page est valide
    if((pte & VALID_BIT) != VALID_BIT) {
        PAGE_FAULT(vaddr, "Page not valid");
        return -1;
    }

    // Lecture des bits RWX
    uint16_t accessBits = (pte & RWX_BITS) >> RWX_POS;

    // Vérification des droits de chaque page
    if(accessBits == W_PATTERN || accessBits == X_PATTERN) {
        PAGE_FAULT(vaddr, "Page can't be read");
        return -1;
    }
    ...
    *byte = main_mem[pte >> SHIFT_RIGHT][((uint16_t)vaddr) & MASK];
    return 0;
}
...
int store_byte(uint32_t vaddr, uint8_t byte) {
    ...
    // Vérification du bit de validité
    if((pte & VALID_BIT) != VALID_BIT) {
        PAGE_FAULT(vaddr, "Page not valid");
        return -1;
    }
    ...
    // Vérification des droits de chaque page
    if(accessBits != RWX_PATTERN  && accessBits != W_PATTERN) {
        PAGE_FAULT(vaddr, "Execute or read only page");
        return -1;
    }
    ...
    // Ecriture mémoire
    main_mem[pte >> SHIFT_RIGHT][((uint16_t)vaddr) & MASK] = byte;
